                ...
                Ok(mut data_ser) => {
                    ...
                    if is_valid(&data_ser.device, hash.clone()) {
                        ...
                        match channel.channel.write_signed(
                            false,
                            PayloadBuilder::new().public(&message).unwrap().build(),
                        ) {
                ...
                Ok(mut data_ser) => {
                    ...
                    if is_valid(&data_ser.device, hash.clone()) {
                        ...
                        if channel.channel.can_send_masked() {
                            match channel.channel.write_signed(
                                true,
                                PayloadBuilder::new().masked(&message).unwrap().build(),
                            ) {
                            ...
                            Ok(HttpResponse::Unauthorized().json("Action invalid: No encryption key generated by subscribers"))
                        }
                    } else {
                        println!(
